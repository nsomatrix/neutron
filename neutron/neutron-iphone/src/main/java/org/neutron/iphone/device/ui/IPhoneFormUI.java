...
public class IPhoneFormUI extends AbstractUI<Form> implements FormUI {
	...
	public IPhoneFormUI(Neutron neutronlator, Form form) {
		super(neutronlator, form);
	}
	...
	public void showNotify() {
		...
		if (view == null) {
			...
			navigtionBar = new UINavigationBar().initWithFrame$(new CGRect(0, 0,
					neutronlator.getWindow().bounds().size.width, NAVIGATION_HEIGHT));
			...
			tableView = new UITableView().initWithFrame$style$(new CGRect(0, NAVIGATION_HEIGHT, neutronlator
					.getWindow().bounds().size.width, neutronlator.getWindow().bounds().size.height
