...
import org.neutron.iphone.device.IPhoneMutableImage;
...
//			CoreGraphics.CGContextRestoreGState(context);
	...
	public IPhoneCanvasUI(Neutron neutronlator, final Canvas canvas) {
		super(neutronlator, canvas);
	}
	...
	public void showNotify() {
		if (view == null) {
			canvasView = new CanvasView(displayable).initWithFrame$(neutronlator.getWindow().bounds());

			view = new UIView().initWithFrame$(neutronlator.getWindow().bounds());
			// tableView = new UITableView().initWithFrame$style$(
			// new CGRect(0, 0, neutronlator.getWindow().bounds().size.width,
			// neutronlator.getWindow().bounds().size.height - 40), 0);
			view.addSubview$(canvasView);
			toolbar = (UIToolbar) new UIToolbar().initWithFrame$(new CGRect(0,
					neutronlator.getWindow().bounds().size.height - TOOLBAR_HEIGHT,
					neutronlator.getWindow().bounds().size.width, TOOLBAR_HEIGHT));
			view.addSubview$(toolbar);
			updateToolbar();
		}
		neutronlator.getWindow().addSubview$(view);
	}
