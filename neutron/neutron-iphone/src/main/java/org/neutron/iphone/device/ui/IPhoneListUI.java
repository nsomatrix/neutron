...
public class IPhoneListUI extends AbstractUI<List> implements ListUI {
	...
	public IPhoneListUI(Neutron neutronlator, List list) {
		super(neutronlator, list);
	}
	...
	public void showNotify() {
		...
		if (view==null) {
			...
			tableView = new UITableView().initWithFrame$style$(
					new CGRect(0, NAVIGATION_HEIGHT, neutronlator.getWindow().bounds().size.width,
							neutronlator.getWindow().bounds().size.height - NAVIGATION_HEIGHT - TOOLBAR_HEIGHT), 0);
			...
			toolbar = (UIToolbar) new UIToolbar().initWithFrame$(new CGRect(0,
					neutronlator.getWindow().bounds().size.height - TOOLBAR_HEIGHT, neutronlator.getWindow().bounds().size.width,
					TOOLBAR_HEIGHT));
