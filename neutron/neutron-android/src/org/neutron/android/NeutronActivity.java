 ...
 *  limitations.
 *
 *  @version $Id: NeutronActivity.java 1918 2009-01-21 12:56:43Z neutron $
 */
...
import org.neutron.android.device.AndroidDeviceDisplay;
import org.neutron.android.device.AndroidFontManager;
import org.neutron.android.device.AndroidInputMethod;
import org.neutron.android.util.ActivityResultListener;
...
public abstract class NeutronActivity extends Activity {
	...
	protected void onCreate(Bundle icicle) {
        ...
        emulatorContext = new EmulatorContext() {
            ...
            private DeviceDisplay deviceDisplay = new AndroidDeviceDisplay(NeutronActivity.this, this, width, height);
            ...
            public DisplayComponent getDisplayComponent() {
                ...
                System.out.println("Neutron.emulatorContext::getDisplayComponent()");
                return null;
            }
            ...
            public InputStream getResourceAsStream(Class origClass, String name) {
                try {
                    if (name.startsWith("/")) {
                        return NeutronActivity.this.getAssets().open(name.substring(1));
                    } else {
                        Package p = origClass.getPackage();
                        if (p == null) {
                            return NeutronActivity.this.getAssets().open(name);
                        } else {
                        	String folder = origClass.getPackage().getName().replace('.', '/');
                            return NeutronActivity.this.getAssets().open(folder + "/" + name);
                        }
                    }
                } catch (IOException e) {
                    Logger.debug(e);
