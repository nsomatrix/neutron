...
package org.neutron.app.ui.swt;
...
import org.neutron.device.swt.SwtButton;
import org.neutron.device.swt.SwtDeviceDisplay;
import org.neutron.device.swt.SwtImmutableImage;
...
public class SwtDeviceComponent extends Canvas {
	...
	MouseAdapter mouseListener = new MouseAdapter() {
		public void mouseDown(MouseEvent e) {
			...
			Device device = DeviceFactory.getDevice();
			org.neutron.device.impl.Rectangle rect = ((SwtDeviceDisplay) device.getDeviceDisplay())
					.getDisplayRectangle();
			...
			if (rect.x <= e.x && (rect.x + rect.width) > e.x && rect.y <= e.y && (rect.y + rect.height) > e.y) {
				if (device.hasPointerEvents()) {
					...
					if (fullScreenMode) {
						...
						org.neutron.device.impl.Rectangle pb = ((SwtDeviceDisplay) device.getDeviceDisplay())
								.getDisplayPaintable();
	...
	public void paintControl(PaintEvent pe) {
		...
		try {
			...
			org.neutron.device.impl.Rectangle displayRectangle = ((SwtDeviceDisplay) device.getDeviceDisplay())
					.getDisplayRectangle();
			...
			org.neutron.device.impl.Rectangle rect;
			if (prevOverButton != null) {
	...
	private void drawImageInShape(SwtGraphics g, Image image, org.neutron.device.impl.Shape shape) {
	...
	private SwtButton getButton(int x, int y) {
		for (Enumeration e = DeviceFactory.getDevice().getButtons().elements(); e.hasMoreElements();) {
			...
			if (button.getShape() != null) {
				try {
					org.neutron.device.impl.Shape tmp = (org.neutron.device.impl.Shape) button.getShape().clone();
					if (tmp.contains(x, y)) {
						return button;
					}
